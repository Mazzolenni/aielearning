<?php
	...
	if ( isset($_REQUEST['subir']) && !empty($_REQUEST['subir']) ) {
		...
		$consulta16 ="INSERT INTO archivos (nombre, tipo, tamanio, usuario) VALUES ('$na','$tipo','$t', '$user'); SELECT @@IDENTITY  AS id;";
		$ejecutar16 = sqlsrv_query ($conn, $consulta16);
		move_uploaded_file ($_FILES['archivo']['tmp_name'], "archivos/".$user.$na);
	}

	$consulta2 = "SELECT * FROM usuario WHERE usuario='".$_SESSION['user']."' ORDER BY fecha DESC";
	$params2 = array();
...
<table>
	...
	if($row_count2>0){
		do{
			...
			echo "<td><a href='archivos.php?eli=".$a2['idusuario']."'>Eliminar</a></td>";

			if(isset($_REQUEST['eli'])){
				$consulta3 = "DELETE FROM archivos WHERE idarchivo =".$_REQUEST['eli'];
				$params3 = array();
